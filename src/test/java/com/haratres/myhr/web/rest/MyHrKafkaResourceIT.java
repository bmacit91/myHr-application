package com.haratres.myhr.web.rest;
...
import com.haratres.myhr.config.KafkaProperties;
...
class MyHrKafkaResourceIT {
    ...
    void setup() {
        ...
        MyHrKafkaResource kafkaResource = new MyHrKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/my-hr-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/my-hr-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
